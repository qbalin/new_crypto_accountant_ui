...
const syncBscData = async (account: Account) => {
  const results = await fetch(`/api/raw_data/${SupportedBlockchain.BinanceSmartChain}?apiKey=${account.blockchainExplorerApiKey}&walletAddress=${account.walletAddress}`).then(res => res.json());
